...
const defaultRooms = [
    { title: "Geral", description: "Um espaço para discussões amplas e variadas.", roomParam: "general", category: "Casual", status: "Ativa", members: 845, date: "12 Jan, 2024" },
    { title: "Tecnologia", description: "Discuta as últimas inovações e tendências.", roomParam: "tech", category: "Tecnologia", status: "Ativa", members: 1204, date: "05 Fev, 2024" },
    { title: "Games", description: "Ponto de encontro para jogadores.", roomParam: "gaming", category: "Jogos", status: "Ativa", members: 432, date: "18 Fev, 2024" },
    { title: "Clube da Música", description: "Discussões sobre música e áudio.", roomParam: "cg", category: "Arte", status: "Arquivada", members: 0, date: "10 Mar, 2024" },
    { title: "Dev Web", description: "Desenvolvimento Frontend e Backend.", roomParam: "web", category: "Tecnologia", status: "Ativa", members: 612, date: "22 Mar, 2024" },
    { title: "Computação Gráfica", description: "Discussões sobre computação gráfica.", roomParam: "cg", category: "Tecnologia", status: "Ativa", members: 200, date: "22 Mar, 2024" }
];
...
const Rooms = () => {
    ...
    const filteredRooms = allRooms.filter(room => room.title.toLowerCase().includes(searchQuery.toLowerCase()));

    return (
        <>
            {isModalOpen && (
                <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/40 backdrop-blur-sm animate-fade-in" onClick={() => setIsModalOpen(false)}>
                    <div className="skeuo-panel p-8 max-w-[450px] w-full relative" onClick={e => e.stopPropagation()}>
                        ...
                        <form onSubmit={handleCreateRoom} className="space-y-4">
                            <div className="space-y-1">
                                ...
                                <input type="text" required value={newRoomTitle} onChange={(e) => setNewRoomTitle(e.target.value)} className="skeuo-input w-full px-4 py-3" />
                            </div>
                            <div className="space-y-1">
                                <label className="block text-[12px] font-medium text-[#86868b] uppercase tracking-widest ml-1">Categoria</label>
                                <select value={newRoomCategory} onChange={(e) => setNewRoomCategory(e.target.value)} className="skeuo-input w-full px-4 py-3 bg-white">
                            ...
                            <div className="space-y-1">
                                ...
                                <input type="text" value={newRoomDesc} onChange={(e) => setNewRoomDesc(e.target.value)} className="skeuo-input w-full px-4 py-3" />
                            </div>
                            <div className="flex gap-3 pt-4">
                                <button type="button" onClick={() => setIsModalOpen(false)} className="btn-secondary-glossy w-full py-3">Cancelar</button>
                                <button type="submit" className="skeuo-btn w-full py-3 text-[16px]">Criar Sala</button>
                            </div>
                        </form>
                    </div>
                </div>
            ...
            <main className="reveal max-w-[1200px] mx-auto p-4 md:p-8 w-full relative space-y-8">
                ...
                <div className="skeuo-panel p-0 overflow-hidden">
                    <div className="p-6 md:p-8 flex flex-col md:flex-row justify-between items-center border-b border-[#d2d2d7]/50 gap-4">
                        ...
                        <button onClick={() => setIsModalOpen(true)} className="skeuo-btn px-6 py-2.5 text-[15px] flex items-center gap-2">
                            <Plus size={18} />
                            Criar Sala
                        </button>
                    </div>
                        ...
                        <table className="w-full text-left border-collapse">
                            ...
                            <tbody>
                                {filteredRooms.map((room, i) => (
                                    <tr key={i} className="border-b border-[#d2d2d7]/30 hover:bg-black/[0.02] transition-colors">
                                        <td className="px-6 py-4">
                                            <div className="flex items-center gap-4">
                                                <div className="relative group flex items-center">
                                                    <div className="w-10 h-10 rounded-[12px] bg-gradient-to-br from-[#0071e3] to-[#4da4ff] text-white flex items-center justify-center shrink-0 shadow-sm border border-[#005bb5] cursor-help transition-all duration-200 hover:scale-105 active:scale-95">
                                                        <Hash size={20} />
                                                    </div>
                                                    {/* Tooltip elegante posicionado à direita para evitar corte físico */}
                                                    <div className="absolute left-[calc(100%+12px)] top-1/2 -translate-y-1/2 flex items-center z-50 pointer-events-none opacity-0 -translate-x-2 group-hover:opacity-100 group-hover:translate-x-0 transition-all duration-300 ease-out w-max max-w-[280px]">
                                                        {/* Seta indicativa 3D apontando para a esquerda */}
                                                        <div className="w-2.5 h-2.5 bg-white dark:bg-[#f4f5f7] border-l border-b border-black/5 rotate-45 -mr-1 shadow-sm z-10"></div>
                                                        {/* Corpo do balão */}
                                                        <div className="skeuo-panel px-3.5 py-2 text-[12px] leading-snug text-[#1d1d1f] font-medium shadow-lg rounded-[12px] border border-black/5 max-w-[250px] break-words text-left">
                                        ...
                                        <td className="px-6 py-4 text-[15px] text-[#424245]">{room.category || "Casual"}</td>
                                        <td className="px-6 py-4">
                                        ...
                                        <td className="px-6 py-4 text-[15px] font-medium text-[#1d1d1f]">{room.members === 0 ? '--' : room.members}</td>
                                        <td className="px-6 py-4">
                                            {room.status !== "Arquivada" && (
